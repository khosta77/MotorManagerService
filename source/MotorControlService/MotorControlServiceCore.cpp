...
void print( const std::vector<uint32_t> &values )
{
    if ( values.size() != ( MOTOR_COUNT * MOTOR_CELLS_COUNT ) )
        throw std::length_error( std::format(
            ...
            values.size(), ( MOTOR_COUNT * MOTOR_CELLS_COUNT ) ) );
...
MotorControlServiceCore::MotorControlServiceCore(
    const std::shared_ptr<ModuleFT232RL> &m_ )
    : UniversalServerCore( "MotorControlServiceCore" ), module_( m_ )
{
}
...
void MotorControlServiceCore::Process( const int id, const std::string &name,
                                       const std::string &msg )
{
    ...
    {
        std::cout << std::format( ">>>> Баг в сообщении: {}", msg )
                  << std::endl;
    ...
    {
        std::cout << std::format( "===> Баг в сообщении: {}", message_in.text )
                  << std::endl;
        // pkg::ImOkay message_err;
        // message_err.status = INT_MAX;
        // writeToSock( id, serialize( message_err ) );
    ...
    std::vector<uint32_t> commandsMessageToBoard(
        MOTOR_COUNT * MOTOR_CELLS_COUNT, 0 );
...
void MotorControlServiceCore::checkMotorNum( const uint32_t num,
                                             const std::string &name )
{
    if ( num > 16 or num < 1 )
        throw std::invalid_argument(
            std::format( "[MotorControlServiceCore]({}): motor "
                         "num = {}, must be > 0 and < 17",
                         name, num ) );
}

void MotorControlServiceCore::setMotor( std::vector<uint32_t> &values,
                                        const size_t i, const uint32_t settings,
...
void MotorControlServiceCore::setMotors( std::vector<uint32_t> &values,
                                         const size_t i,
 ...
 * */
void MotorControlServiceCore::setCAE( std::vector<uint32_t> &values,
                                      const mcs::MotorsGroupSettings &msg )
{
    if ( !msg.configureAllEngines.empty() )
    {
        for ( const auto &group : msg.configureAllEngines )
        {
            ...
            for ( const auto &g_ : group.motors )
                setMotors( values, ( ( g_ - 1 ) * MOTOR_CELLS_COUNT ), settings,
                           group );
        }
    }
}
 ...
 * TODO(khosta77) : прописать в КД, коллизию выше
 * */
void MotorControlServiceCore::setSS( std::vector<uint32_t> &values,
                                     const mcs::MotorsGroupSettings &msg )
{
    if ( !msg.startSimultaneously.empty() )
    {
        ...
        for ( const auto &g_ : groups )
            setMotor( values, ( ( g_.motor - 1 ) * MOTOR_CELLS_COUNT ),
                      settings, g_ );
    }
}
 ...
 * TODO(khosta77) : прописать в КД, коллизию выше
 * */
void MotorControlServiceCore::setSI( std::vector<uint32_t> &values,
                                     const mcs::MotorsGroupSettings &msg )
{
    if ( !msg.startImmediately.empty() )
    {
        for ( const auto &g_ : msg.startImmediately )
        {
            ...
            setMotor( values, ( ( g_.motor - 1 ) * MOTOR_CELLS_COUNT ),
                      ( ( 1U << ( 32 - static_cast<uint32_t>( g_.motor ) ) ) |
                        0x2000 ),
                      g_ );
        }
    }
